...
namespace exxen2._0.capaLogica
{
    ...
    public class AsistenciaLogica
    {
        ...
        public Asistencia Registrar(Asistencia asistencia)
        {
            ...
            {
                var socio = context.Socios.Buscar(asistencia.IdSocio);
                if (socio == null || !socio.Estado)
                {
                    throw new InvalidOperationException("El socio no existe o está inactivo.");
                }

                var membresia = context.Membresias.Where(m => m.IdSocio == asistencia.IdSocio).OrderByDescending(m => m.FechaInicio).FirstOrDefault();
                if (membresia == null)
                {
                ...
                var cuota = context.CuotasMembresia.Where(c => c.IdMembresia == membresia.IdMembresia && c.FechaDesde < siguienteDia && c.FechaHasta >= inicioDia && c.EstadoPago != EstadosCuota.Anulada).SingleOrDefault();
                ...
                context.Asistencias.Agregar(asistencia);
        ...
        public List<Asistencia> ListarPorSocio(int idSocio)
        {
            using (var context = new GymUnidadDeTrabajo())
            {
                return context.Asistencias.ConsultarSoloLectura().Where(a => a.IdSocio == idSocio && a.Estado).OrderByDescending(a => a.Fecha).ToList();
            }
        }
        ...
        public List<Asistencia> ListarPorFecha(DateTime fecha)
        {
            ...
            using (var context = new GymUnidadDeTrabajo())
            {
                return context.Asistencias.ConsultarSoloLectura("Socio").Where(a => a.Estado && a.Fecha >= inicio && a.Fecha < fin).OrderBy(a => a.Fecha).ToList();
            }
        }
        ...
        public List<Asistencia> ListarPorFechaParaGestion(DateTime fecha)
        {
            ...
            using (var context = new GymUnidadDeTrabajo())
            {
                return context.Asistencias.ConsultarSoloLectura("Socio").Where(a => a.Fecha >= inicio && a.Fecha < fin).OrderByDescending(a => a.Estado).ThenBy(a => a.Fecha).ToList();
            }
        }
        ...
        public void DarDeBaja(int idAsistencia)
        {
            using (var context = new GymUnidadDeTrabajo())
            {
                var asistencia = context.Asistencias.Buscar(idAsistencia);
                if (asistencia == null)
                {
