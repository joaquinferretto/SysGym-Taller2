...
## Reparacion de inicializacion de RutinasEntrenadorFormulario - 18 de septiembre de 2026

Se trabajo exclusivamente sobre `capaVisual/Entrenador/RutinasEntrenadorFormulario.Designer.cs` y la documentacion afectada. El Designer ya no contiene loops ni helpers de configuracion; las cuatro filas del detalle se declaran explicitamente y los botones, grillas, campos, columnas, nombres y eventos quedan representados mediante propiedades normales de Windows Forms. La normalizacion habia omitido `components = new Container();`, por lo que el campo declarado quedaba `null`; se restauro esa inicializacion estandar al inicio de `InitializeComponent`. La pantalla master/detail y sus contenedores funcionales no fueron redisenados. El NRE informado no se reprodujo en la apertura real del diseñador despues de la correccion; no se atribuyeron fallos a handlers sin una traza que lo confirme.
...
Verificaciones realizadas: Debug y Release sin errores; constructor y layout en memoria; cuatro filas declarativas; SplitterDistance valido; estados de botones y campos para Consulta/Nuevo/Edicion; Cancelar Nuevo; auditoria de handlers sin duplicados; apertura real de `RutinasEntrenadorFormulario.cs [Diseño]` en Visual Studio. El constructor conserva unicamente la preparacion del usuario y `InitializeComponent`; `Load` evita consultas en modo diseño. Los warnings CS0649 restantes pertenecen a otros formularios previamente modificados. Pendientes: ejecutar SysGym contra una base de prueba y recorrer Agregar/Editar/Cancelar/Quitar con datos reales.
