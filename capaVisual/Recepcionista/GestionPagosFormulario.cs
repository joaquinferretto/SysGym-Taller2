...
namespace exxen2._0.capaVisual.Recepcionista
{
    ...
    public partial class GestionPagosFormulario : Form
    {
        ...
        private void membresia_SelectedIndexChanged(object origen, EventArgs e)
        {
        ...
        {
            var opcionSeleccionada = membresia.SelectedItem as OpcionMembresiaPago;
            if (opcionSeleccionada == null || !opcionSeleccionada.Habilitada)
            {
                MostrarSinCuota("Selecciona una membresia activa.");
                return;
            }

            var pendiente = cuotasCargadas.Where(c => c.IdMembresia == opcionSeleccionada.IdMembresia && c.EstadoPago == EstadosCuota.Pendiente && !c.IdRegistroPago.HasValue).OrderBy(c => c.FechaDesde).FirstOrDefault();
            if (pendiente == null)
            {
        ...
        private void anular_Click(object origen, EventArgs e)
        {
            try
            {
                ...
                Cargar();
                nuevo_Click(null, EventArgs.Empty);
                AyudaFormularioVisual.MostrarExito(lblEstado, "Pago anulado.");
            }
            catch (Exception ex)
            {
                AyudaFormularioVisual.MostrarError(lblEstado, ex);
        ...
        private void reembolsar_Click(object origen, EventArgs e)
        {
            try
            {
                if (idPagoSeleccionado == 0)
                ...
                Cargar();
                nuevo_Click(null, EventArgs.Empty);
                AyudaFormularioVisual.MostrarExito(lblEstado, "Pago reembolsado.");
            }
            catch (Exception ex)
            {
                AyudaFormularioVisual.MostrarError(lblEstado, ex);
        ...
        private void GestionPagosFormulario_Load(object origen, EventArgs e)
        {
