...
namespace exxen2._0.capaVisual.Recepcionista
{
    ...
    public partial class GestionAsignacionesForm : Form
    {
        ...
        public GestionAsignacionesForm()
        {
            ...
            FormularioVisualHelper.ConfigurarEntradaEntero(membresia);
        ...
        private void Asignar(object sender, EventArgs e) { try { var id = FormularioVisualHelper.Entero(membresia, "membresia"); FormularioVisualHelper.ValidarComboSeleccionado(entrenador, "un entrenador"); var a = logica.AsignarEntrenador(id, Convert.ToInt32(entrenador.SelectedValue)); idSeleccionado = a.IdMembresiaEntrenador; CargarLista(id); FormularioVisualHelper.MostrarExito(lblEstado, "Entrenador asignado."); } catch (Exception ex) { FormularioVisualHelper.MostrarError(lblEstado, ex); } }
        private void Cambiar(object sender, EventArgs e) { try { var id = FormularioVisualHelper.Entero(membresia, "membresia"); FormularioVisualHelper.ValidarComboSeleccionado(entrenador, "un entrenador"); var a = logica.CambiarEntrenador(id, Convert.ToInt32(entrenador.SelectedValue)); idSeleccionado = a.IdMembresiaEntrenador; CargarLista(id); FormularioVisualHelper.MostrarExito(lblEstado, "Entrenador cambiado."); } catch (Exception ex) { FormularioVisualHelper.MostrarError(lblEstado, ex); } }
